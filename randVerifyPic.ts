...
interface OptionModel{
 ...
 class GVerify{
     ...
     */
    constructor(parentId:string,canvasId:string){
    ...
    _init() {
        ...
        var canvas:any = document.getElementById(this.options.canvasId); //获得画布  IE不能支持canvas，可以增加excanvas.js插件，但是还是不支持createelement()的形式
     ...
     /**生成验证码**/
     refresh() {
        var canvas:any = document.getElementById(this.options.canvasId); //获得验证码画布
   ...
   width:矩形长度
   height:矩形高度*/
        let codeArr  = this.options.code.split('')
    ...
    /**
    * 验证验证码
    * @param {String} code     //验证验证码字符串  E.g: 1234
    */
